// Updated End-to-End Test Script with fixed selector
...
(async () => {
  ...
  try {
    ...
    // Test 3: Test dark mode toggle - FIXED SELECTOR
    console.log('\nTest 3: Testing dark mode toggle...');
    const initialTheme = await page.evaluate(() => {
      return document.documentElement.classList.contains('dark');
    });
    
    // Fixed selector to match the actual button in the HTML
    await page.click('button[aria-label="Toggle dark mode"], button[class*="text-gray-500"]');
    ...
    await new Promise(resolve => setTimeout(resolve, 500)); // Wait for Alpine.js to update the DOM
